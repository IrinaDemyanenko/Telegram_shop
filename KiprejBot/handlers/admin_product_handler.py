...
from keyboards.admin_keyboards import product_menu, get_category_keyboard
...
@admin_router_product_handler.message(F.content_type.in_({ContentType.PHOTO, ContentType.TEXT}))
...
@admin_router_product_handler.message(AddProduct.variants)
...
@admin_router_product_handler.message(Command("done"))
async def confirm_variants(message: Message, state: FSMContext):
    async with state.proxy() as data:
        if "variants" not in data or not data["variants"]:
            ...
            return
    await message.answer("Все варианты добавлены! Теперь подтвердите сохранение товара.", reply_markup=product_menu)
...
@admin_router_product_handler.message(AddProduct.confirm)
...
@admin_required
async def add_product_start(message: Message, state: FSMContext, session: AsyncSession):
    await message.answer("✅ Посмотреть товар!", reply_markup=product_menu)


@admin_router_product_handler.message(F.text == "Редактировать товар")
@admin_required
async def add_product_start(message: Message, state: FSMContext, session: AsyncSession):
    await message.answer("✅ Редактировать товар", reply_markup=product_menu)


@admin_router_product_handler.message(F.text == "Удалить товар")
@admin_required
async def add_product_start(message: Message, state: FSMContext, session: AsyncSession):
    await message.answer("✅ Удалить товар!", reply_markup=product_menu)
